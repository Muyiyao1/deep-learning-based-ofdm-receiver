...
所有图和表均由 `results/final/` 的当前 CSV 自动生成。BER 曲线使用对数坐标，以均值曲线和半透明 95% Student-t 置信带展示不确定性；方法较多的 practical 对比改为按 SNR 分列的点-区间图，避免重叠折线。若有限次 Monte Carlo 样本得到的下置信限小于等于 0，则该段置信带不会被伪画到任意小的正数，而是在图中省略并注明。仅重绘图像可运行：`python scripts/regenerate_result_figures.py --results-dir results/final`。
