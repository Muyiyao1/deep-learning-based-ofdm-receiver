...
所有图和表均由 `results/final/` 的当前 CSV 自动生成。BER 使用对数坐标，图中展示均值和 95% Student-t 置信区间；若有限次 Monte Carlo 样本得到的下置信限小于等于 0，则该下侧误差条不会被伪画到任意小的正数，而是在图中省略并注明。仅重绘图像可运行：`python scripts/regenerate_result_figures.py --results-dir results/final`。
